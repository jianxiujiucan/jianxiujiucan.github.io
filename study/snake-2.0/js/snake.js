...
class Snake {
  constructor(params) {
    ...
    this.width = params.width || 400; // 游戏区域的宽度
    this.height = params.height || 400; // 游戏区域的高度
    this.colorFood = params.colorFood || "orange"; //食物颜色
    this.colorSnake = params.colorSnake || "#666"; //蛇身颜色
    this.colorHead = params.colorHead || "red"; //蛇头颜色
    this.snake = []; // 蛇的身体
  ...
  reset() {
    ...
    canvas.style.background = `repeating-linear-gradient(transparent 0 ${this.size - 1}px, #ccc ${this.size}px), repeating-linear-gradient(to right, transparent 0 ${this.size - 1}px, #ccc ${this.size}px)`
  ...
  _moving() {
    ...
    this.timer = setInterval(() => {
      ...
      // 判断是否撞墙
      if (
        x > this.width / this.size - 1 || x < 0 || y > this.height / this.size - 1 || y < 0
      ) {
   ...
   */
  _createFood() {
    const index = this._getRandom(0,this.grid.length - 1)
    this.food = this.grid[index]
    console.log('食物坐标:' + this.food, '食物长度：' + this.grid.length)
    this.snake.every((item) => {
      if (this.food[0] === item[0] && this.food[1] === item[1]) {
        return this._createFood();
      }
    });
    this.grid.splice(index,1)
    console.log(`删除第${index}个`,this.grid)
  }
  /**
   * 获取方向
   */
